...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def update_preferences(self, request):
        ...
        for field in [
            'prefers_segmented_sessions', 'prefers_extra_time',
            'prefers_text_communication', 'prefers_visual_aids',
            'prefers_literal_language', 'prefers_dyslexia_formatting'
        ]:
...
from .serializers import CompanySerializer, CompanyDetailSerializer, JobPositionSerializer
...
from .serializers import (
    ProblemSerializer, AssessmentSerializer, CandidateAssessmentSerializer,
    SubmissionSerializer, ProblemDetailSerializer, AssessmentDetailSerializer
)
from users.permissions import IsCompanyAdmin
...
class AssessmentViewSet(viewsets.ModelViewSet):
    ...
    def assign_candidates(self, request, pk=None):
        ...
        return Response({'status': 'candidates assigned'}, status=status.HTTP_201_CREATED)
...
class CandidateAssessmentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if admin_companies.exists():
            # Company admins see their company's candidate assessments
            company_ids = admin_companies.values_list('id', flat=True)
            return CandidateAssessment.objects.filter(assessment__company_id__in=company_ids)
